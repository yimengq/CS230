...
"Some info about GANS. You have a generator network, that generates music synthesis. You have also a discriminator"
"network, that detects if the output of the generator is fine or not. Especially, is the synthesis"
"sounding like a human audio file as I was taught it was?"

"Some information about the super method:  https://docs.python.org/2/library/functions.html#super. All those functions are custom functions" 
"to make the code works even though all the requirements are not satisfied for using pytorch functions"
class GLU(nn.Module):
    def __init__(self):
...
"Definition of the CycleGAN Generator & Discriminator"
##########################################################################################
...
##########################################################################################
"Class is a subset of functions that are going to be applicable only if you call themby their name. In the case"
"Just below, if I want to call the forward function, I will have to wirte Generator.forward(input)"

"Super = call previous classes to reuse them with all the parameters"

"self does Generator.forward, etc... on the class itself"
